...
// The access result the HerculesIAM provider and the /auth/* pages consume.
...
// evaluateAccess - run once after sign-in (deployment entry, see the
// HerculesIAM provider) and from "Check again". Performs deployment entry via
// access.enter (the tenant's access mode decides admit / pending / deny), then
// normalizes the outcome, using the mirror to distinguish an existing
// blocked/suspended/removed membership from a policy denial. Multi-tenant apps
// can add a tenant arg passed through to access.enter and
// access.me.accessStatus.
