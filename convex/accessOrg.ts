...
export function activeOrgMemberships(memberships: AccessMembership[]): OrgMembership[] {
...
export function findActiveOrgMembership(
  ...
  return activeOrgMemberships(memberships).find((membership) => membership.scopeId === scopeId);
}
...
export async function requireActiveOrgMembership(
  ...
  const membership = findActiveOrgMembership(await listMyMemberships(ctx), scopeId);
  if (!membership) {
